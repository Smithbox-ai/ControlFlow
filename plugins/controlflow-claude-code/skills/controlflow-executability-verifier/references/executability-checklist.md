...
Use this grid during the cold-start simulation in
...
| --- | --- | --- |
| `what_clear` | The task objective is stated with no ambiguity | Objective uses vague terms without a concrete target |
| `where_clear` | Exact file paths, modules, or components are identified | Phase says "relevant files" or relies on executor to guess |
...
| `inputs_defined` | All required inputs (data, config, environment, upstream artifacts) are named | Phase depends on a prior output that is not explicitly described |
...
| `verify_command_complete` | A runnable verification command is given | Phase says "run the tests" without specifying which command |
...
For each simulated task, mentally execute this sequence and stop at the first real
blocker:

1. `open_file` - can you locate the exact file? If the file does not exist, is there a
   create instruction?
2. `read_existing_code` - does the plan acknowledge the existing implementation? Does it
   say what to preserve or discard?
...
5. `write_implementation_green` - can the implementation be written without guessing API
   names, signatures, or patterns not stated in the plan?
...
Stop at the first hard blocker in a task and record exactly why execution would stall.
Do not continue past a hard blocker with assumed context.
