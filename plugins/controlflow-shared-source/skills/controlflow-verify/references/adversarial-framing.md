...
Inline verification lacks the isolation of a fresh agent context, so confirmation bias is the primary risk. These rules mitigate it.
...
- When self-review bias is unacceptable (LARGE, high blast radius), recommend the user run native Copilot code review in a fresh context for an isolated second pass.
