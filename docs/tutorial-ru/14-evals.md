...
1. **(новичок)** Запустите `cd evals && npm install && npm test`. Сколько тестов прошло? При желании перенаправьте вывод в локальный файл (`npm test > out.txt`, он в .gitignore), чтобы просмотреть последний прогон.
