...
- **Spec capture** — компактный spec-before-plan artifact для расплывчатых или нетривиальных `SMALL+` задач.
...
Для расплывчатых задач и нетривиальных `SMALL`, `MEDIUM` или `LARGE` планов Planner создает компактный spec artifact по [`plans/templates/spec-template.md`](../../plans/templates/spec-template.md) и записывает `spec_path` в план. Это first-class spec-before-plan checkpoint; TRIVIAL only-when-obvious задачи могут его пропускать.
...
- `phase_task_card_path` — optional compact executor task card, обязательная практика для `resource_profile: small_local`.
...
1. **(новичок)** Откройте `Planner.agent.md` и найдите 10 шагов workflow. Сравните с диаграммой выше.
